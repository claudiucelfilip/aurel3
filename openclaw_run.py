...
def build_agent_message(task_payload: dict) -> str:
    return "\n".join([
        ...
        "source_item_id, market_relevant, event_type, theme_id, theme_label, summary, beneficiary_sectors, hurt_sectors, direct_beneficiaries, secondary_beneficiaries, ticker_impacts, time_horizon, durability, confidence, actionability, reasoning_notes",
        "ticker_impacts must contain one object per named ticker: {ticker, direction, rationale}.",
        "Allowed ticker impact direction: bullish | bearish | mixed | neutral.",
        "Directness means causal proximity, not bullishness. Lawsuits, deal-blocking actions, cuts, misses, and sector selloffs are bearish or mixed for affected tickers.",
